...
if (!isset($_REQUEST['oauth_token'])) {
	...
	$request_token = $connection->getRequestToken("http://" . sitedomain . "/lib/oauth_login.php?method=twitter&referer=" . urlencode($_SERVER['REQUEST_URI']));
...
if (isset($_REQUEST['oauth_token'])) {
	...
	if ($conn->http_code==200) {
		...
		$twitterme = $twitter->get('account/verify_credentials');	
...
function Tweet($texto,$access_token=siteTwitterAccessToken,$acces_token_secret=siteTwitterAccessTokenSecret) {
	if ((siteBitlyEnabled) and (is_file("bitly.php"))) { 
		require_once("bitly.php"); 
		$Palabras=explode(' ',$texto);
		$salida="";
		foreach ($Palabras as $vocablo) {
			if ((strpos($vocablo,'http')!==false) and (strpos($vocablo,'bit.ly')===false) and (siteBitlyEnabled)) {
				$bitly = new Bitly(siteBitlyUser, siteBitlyAPIKey);
				$vocablo=$bitly->shorten($vocablo);
				unset($bitly);
			}
			$salida.=$vocablo . " ";
		}
	...
	$parameters = array('status' => $texto); // utf8_encode($texto)); 
