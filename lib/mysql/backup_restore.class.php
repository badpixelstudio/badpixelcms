...
//	    mysql_set_charset( DB_CHARSET, $this->db );
	    ...
	    $tables = mysqli_query($this->conexion, 'SHOW TABLES' );
	    ...
	    if ($totalRows_tables>0) {
	    	do {
				$curr_table = $row_tables[mysqli_fetch_field_direct( $tables,0 )->name];
	...
	//Restore
 	public function restore($file = NULL) {
			...
			foreach ($lines as $line) {				
				...
				// Multiline query
				    ...
				    continue;
 					...
 					$sql.=$line . "\r\n" ;
					$result = @mysqli_query($this->conexion,$line) or false;					
					if (!$result ) {							
						$buffer=$line;
						continue;
						//echo mysql_error();
		...
		$this->msg ="Se ha restaurado la copia de seguridad";			
		return $this->msg;
 	}
	...
	private function make_sql( $sql_file, $table ) {
	    ...
	    for ( $j = 0; $j < $fields_cnt; $j++ ) {
	    	...
	    	$field_set[$j] = $this->sql_backquote(  mysqli_fetch_field_direct( $result, $j )->name );
