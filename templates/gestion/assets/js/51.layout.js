...
var Layout = function () {
    ...
    var handleMenuSelected = function() {
       //Autoselect item-menu. By Israel García. 2016-03-04
       var panfold=$('#PanelFolder').attr('name');
       var loc=location.pathname;
       loc=loc.substr(loc.indexOf(panfold)+panfold.length+1,loc.length);
       var pos=loc.indexOf("/");
       if (pos!=-1) { loc=loc.substr(0,loc.indexOf("/")); }
       //var loc = location.pathname.replace(/\/([^.]+)\//g,'');
       var current = $('#nav').find('a[href="'+loc+'"]');
        
        if(current.parent().parent().is('#nav')){
            current.parent().addClass('active');
            current.parent().find('a').append('<span class="selected"></span>');
        }else{
            current.parent().parent('ul').parent('li').addClass('active');
            current.parent().parent('ul').parent('li').find('a').append('<span class="selected"></span>');
            current.parent().parent('ul').parent('li').find('.arrow').addClass('open');
            current.parent().addClass('active');
        }   
    }
    ...
    return {
        ...
        init: function () {
            ...
            handleMenuSelected(); //handles auto-select item main menu 
        },
