    ...
    private function getURIFromEnvironment($options)
    {
        ...
        if (!$uri) { // use given URI in config options
            if (isset($_SERVER['REDIRECT_URL']) && isset($_SERVER['SCRIPT_NAME'])) { // use redirection URL from Apache environment
                ...
                //Modified by I.Garcia (israel@badpixel.es) for run in PHP 5.6.15
                $server=$_SERVER['REDIRECT_URL'];
                $uri = substr($server, strpos($server,$dirname)+strlen($dirname));
                //$uri = substr($server, strlen($dirname == DIRECTORY_SEPARATOR ? '' : $dirname));
        ...
        //Modified by I.Garcia (israel@badpixel.es) Patch for ? in url_request
        $uri=explode("?",$uri);
        $uri=$uri[0];
        // get mimetype
        $parts = explode('/', $uri);
